...
char *getTimestamp(){
    ...
    snprintf(strTimestamp, sizeof(strTimestamp), "(%02d:%02d:%02d) ", local->tm_hour, local->tm_min, local->tm_sec); // ercit printf dans strTimestamp  
    printf("\nmytime : %s \n\n", strTimestamp);
...
void *sending_msg(void *socket) {
    ...
    char* strTimestamp = getTimestamp();

    // Message buffer
    char buffer[1024];

    while (fgets(buffer, 1024, stdin)) {
        ssend(clientfd, buffer, strlen(buffer) + 1, strTimestamp);
...
int main(int argc, char *argv[]) {
    ...
    // Send pseudo
    ssend(clientfd, pseudo, strlen(pseudo) + 1, strTimestamp);

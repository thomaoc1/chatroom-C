...
void message_send(int socket, const char* message, const char* pseudo) {

    // Save the sender of the message
    int sender = socket;
...
void *handle_client(void *socket) {
    ...
    message_send(clientfd, "[connected]\n", pseudo);
    while (nbytes > 0) {
        nbytes = receive(clientfd, (void **)&buffer);
        if (nbytes > 0) {
            message_send(clientfd, buffer, pseudo);
            free(buffer);
        }
    }
    ...
    message_send(clientfd, "[disconnected]\n", pseudo);

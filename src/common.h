...
/**
 * @brief Send data under the form <size_t len><time_t rawtime><...data>
...
size_t receive(int sock, void **message, time_t *now) {
	...
	// Checking how many bytes last read will receive
	size_t nbytes_to_receive;
    if (checked(read(sock, &nbytes_to_receive, sizeof(nbytes_to_receive))) ==
        0) {
        // Connection closed
        return 0;
    };
	...
	// Allocating number of bytes to be received
    unsigned char *buffer = (unsigned char *)malloc(nbytes_to_receive);

	// Checking malloc
    if (buffer == NULL) {
        fprintf(stderr, "malloc could not allocate %zd bytes",
                nbytes_to_receive);
        perror("");
        exit(1);
    }

 ...
 */
void ssend(int sock, void* data, size_t len, char* strTimestamp) {
  //printf("%zd bytes to write\n", len);
  checked(write(sock, &len, sizeof(len)));
  checked(write(sock, strTimestamp, strlen(strTimestamp));
...
size_t receive(int sock, void** dest) {
  ...
  unsigned char* buffer = (unsigned char*)malloc(nbytes_to_receive); //faut pas faire un free apres ???
  if (buffer == NULL) {
